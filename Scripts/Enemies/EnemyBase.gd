...
func _process(delta):
	...
	for i in playerHit:
		...
		if i.get_collision_layer_value(20) or i.supTime > 0 or forceDamage:
			...
			if !i.ground and !i.get_collision_layer_value(24):
				if i.movement.y > 0 and i.global_position.y < global_position.y:
					...
					i.movement.y -= 100
				if i.get_shield() == PlayerChar.SHIELDS.BUBBLE and i.abilityUsed:
					i.emit_enemy_bounce()
					i.bounceReaction = i.defaultBounceReaction
			# destroy
...
func _on_DamageArea_area_entered(area):
	# damage checking
	if area.get("parent") != null and area.get_collision_layer_value(20) and !playerHit.has(area.parent):
		playerHit.append(area.parent)
	if area.get_collision_layer_value(24) and !playerHit.has(Global.players[0]):
		playerHit.append(Global.players[0])
	forceDamage = true
