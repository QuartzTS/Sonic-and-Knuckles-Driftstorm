...
func _process(_delta):
	if isJump:
		...
		if parent.inputs[parent.INPUTS.SUPER] == 1 and !parent.isSuper:
			...
			if parent.rings >= 50 and Global.emeralds >= 127:
				...
				if parent.partner != null:
					parent.partner.set_state(parent.partner.STATES.SUPER)
		# Shield actions
		elif (parent.inputs[parent.INPUTS.ACTION] == 1 or (parent.inputs[parent.INPUTS.ACTION2] == 1 and !parent.character == Global.CHARACTERS.KNUCKLES)) and parent.playerControl != 0 and !parent.abilityUsed:
			# Super actions
							...
							match (parent.shield):
								...
								parent.SHIELDS.BUBBLE:
									...
									if parent.shieldSprite.animation != "BubbleBounce":
										...
										parent.bounceReaction = parent.defaultBounceReaction
		...
		elif parent.playerControl != 0 and parent.character == Global.CHARACTERS.KNUCKLES and parent.inputs[parent.INPUTS.ACTION2] == 1:
			parent.sfx[33].play()
			parent.animator.play("drillDive")
			parent.movement = Vector2(0,12*60)
			parent.set_state(parent.STATES.DRILLDIVE,parent.currentHitbox.NORMAL)
...
func _physics_process(delta):
	...
	parent.defaultBounceReaction = 4.0 if parent.water else 7.5
