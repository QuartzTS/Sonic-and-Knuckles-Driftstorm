...
func _process(delta):
	if parent.inputs[parent.INPUTS.ACTION2] == 1 and !sliding and !landed:
		parent.sfx[33].play()
	...
	if (parent.inputs[parent.INPUTS.ACTION] == 1 or parent.inputs[parent.INPUTS.ACTION2] == 1) and parent.ground and (sliding or isFall):
		parent.movement.x = 0
	...
	if !isFall and !sliding:
		...
		if !parent.any_action_held_or_pressed():
