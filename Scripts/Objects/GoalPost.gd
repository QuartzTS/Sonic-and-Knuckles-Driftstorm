...
func _physics_process(_delta):
	...
	if !Global.is_in_any_stage_clear_phase() and player.global_position.x > global_position.x and player.global_position.y <= global_position.y and \
	Global.get_level_act_number() != 2:
	...
	if Global.is_in_any_stage_clear_phase() and $Sprite.animation == "spinner":
		...
		if Global.get_stage_clear_phase() == Global.STAGE_CLEAR_PHASES.GOALPOST_SPIN_END:
